...
const checkDaysDB = require('../db/checkDaysDB').default
const addDayDB = require('../db/addDayDB').default
...
const messages = [
  ', с твоим Днём! Пусть этот день станет для тебя самым счастливым в году. Желаем эффективного саморазвития и поиска себя!💕',
    ...
    if (server_time >= TIME) {
      ...
      Users.forEach((user) => {
        if (user.bdate !== undefined) {
          if (
            date.getDate() === parseInt(user.bdate.split('.')[0]) &&
            date.getMonth() + 1 === parseInt(user.bdate.split('.')[1])
          ) {
